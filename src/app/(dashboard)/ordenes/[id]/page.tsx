...
export default async function OrderDetailPage({ params }: Props) {
  ...
  const isParking = order.kind === "parking";
  // Parqueo por hora aún sin cobrar: en el header mostramos la tarifa por hora
  // (el total real se calcula al registrar la salida).
  const pendingHour = isParking && order.parkingRateType === "hour" && !isDelivered;
  const headerValue = pendingHour ? order.parkingRate ?? 0 : order.total;
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="bg-card border-b border-border/40 px-4 pt-5 pb-4 md:px-6">
        ...
        <div className="flex items-end justify-between gap-2">
          ...
          <div className="text-right">
            ...
            >
              {formatPrice(headerValue)}
              {pendingHour && <span className="text-sm font-semibold text-muted-foreground">/h</span>}
            </p>
            <div
            ...
            >
              <span aria-hidden="true">{isParking ? "🅿️" : meta.icon}</span>{" "}
              {isParking ? (isDelivered ? "Salió" : "Parqueado") : meta.label}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 bg-background px-4 py-5 space-y-4 md:px-6">
        ...
        {/* Lavado: estado + stepper + servicios */}
        {!isParking && (
          <>
                    ...
                    <div key={step} className="flex items-center gap-1 flex-1 min-w-0">
                      ...
                      {idx < STATUS_STEPS.length - 1 && (
                        <div
                          ...
                          style={{ background: idx < currentStep ? meta.color : "var(--color-border)" }}
                        />
                      )}
                    </div>
                  );
          ...
          <section className="bg-card rounded-2xl border border-border/50 shadow-sm p-4">
            ...
            <div className="flex justify-between items-center py-2">
              ...
              <span className="text-sm font-semibold text-foreground">
                {RATE_TYPE_LABEL[order.parkingRateType ?? "hour"]}
              </span>
            </div>
            <div className="flex justify-between items-center py-2 border-t border-border/40">
              ...
              <span className="text-sm font-bold" style={{ fontFamily: "var(--font-space-mono)" }}>
                {formatPrice(order.parkingRate ?? 0)}
                {order.parkingRateType === "hour" ? "/h" : " /día"}
              </span>
            </div>
            <div className="flex justify-between items-center pt-3 mt-1 border-t border-border/60">
              <span className="text-sm font-bold text-foreground">
                {isDelivered ? "Total cobrado" : "Total"}
              </span>
              <span
                className="text-xl font-extrabold"
                style={{ fontFamily: "var(--font-space-mono)", color: "var(--color-primary)" }}
              >
                {isDelivered || order.parkingRateType === "day"
                  ? formatPrice(order.total)
                  : "Se calcula a la salida"}
              </span>
            </div>
          </section>
        )}
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2.5 text-sm">
            {(isParking
              ? [
                  ...
                  ["Salida",    formatDate(order.deliveredAt)],
                ]
              : [
        ...
        {/* Acción */}
        {!isDelivered && isParking && <ExitButton orderId={order.id} />}
        {!isDelivered && !isParking && (
          <AdvanceButton
            orderId={order.id}
        ...
        {isDelivered && (
          <div className="flex flex-col items-center gap-2 py-4">
            <span className="text-3xl" aria-hidden="true">{isParking ? "🅿️" : "🎉"}</span>
            <p className="text-sm text-muted-foreground font-medium">
              {isParking ? "Parqueo cerrado y cobrado" : "Orden completada y entregada"}
            </p>
            <Link href="/ordenes" className="text-sm font-semibold text-primary">
